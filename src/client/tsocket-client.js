...
SocketClient.prototype.connect = function () {
    ...
    client.connect({
        ...
        thisObj.debug('new connection[Port:%s]', this.localPort);
    });
...
SocketClient.prototype._handleNotification = function (socket, serverMsg, sendMsg) {
    var thisObj = this;
    thisObj.debug('<== serverMsg: %s', serverMsg.encode());
    thisObj.info('<== [%s]: received notification: %s', thisObj.pushAdd, serverMsg.content[cMsg.EMsgKey.sNotification_msg]);
    // write response to server
    sendMsg.type = cMsg.EMsgType.cNotificationRtn;
    thisObj.info('==> return for notification received: %s', sendMsg.encode());
    socket.write(sendMsg.encode());
};

// launch
var host = '127.0.0.1';
//var host = '103.224.234.134';
